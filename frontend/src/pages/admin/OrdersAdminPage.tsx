...
function OrderDrawer({ order, onClose }: { order: Order; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 sm:inset-auto sm:top-0 sm:right-0 sm:h-full sm:w-full sm:max-w-md bg-white z-50 sm:shadow-2xl flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div><span className="text-gray-500 block text-xs">Status</span><p className="font-medium capitalize">{order.status}</p></div>
            <div><span className="text-gray-500 block text-xs">Payment</span><p className="font-medium capitalize">{order.payment_status}</p></div>
            <div><span className="text-gray-500 block text-xs">Amount</span><p className="font-bold text-primary">₹{order.total_amount}</p></div>
            <div><span className="text-gray-500 block text-xs">Date</span><p>{new Date(order.created_at).toLocaleDateString()}</p></div>
          </div>
          <div>
          ...
          <div>
            ...
            <button onClick={handleUpdate} disabled={mutation.isPending || newStatus === order.status}
              className="w-full mt-3 py-3 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary/90 disabled:opacity-60 touch-target">
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1">
        {STATUSES.map((s) => (
          <button
            ...
            className={`px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm rounded-full border whitespace-nowrap transition touch-target ${statusFilter === s ? 'bg-primary text-white border-primary' : 'bg-white hover:border-gray-300'}`}
          >
            {STATUS_LABELS[s]}
          </button>
        ))}
              ...
              data?.items?.map((o: Order) => (
                <tr key={o.id} className="border-b last:border-0 hover:bg-gray-50 cursor-pointer" onClick={() => setSelectedOrder(o)}>
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-xs font-semibold px-2.5 py-1 rounded-full capitalize ${STATUS_COLORS[o.status] || 'bg-gray-100'}`}>{o.status}</span>
                  </td>
      ...
      <div className="sm:hidden space-y-2">
        {isLoading ? (
          ...
          data?.items?.map((o: Order) => (
            <button key={o.id} onClick={() => setSelectedOrder(o)} className="w-full bg-white rounded-xl border p-3 text-left active:scale-[0.99] transition-transform">
